local set = vim.keymap.set
local opts = { noremap = true, silent = true }

-- commonly use
set('n', '0p', [["0p]], opts)

-- set('i', '<esc>', '<Nop>', opts)
set('i', 'jk', '<esc>', opts)

set('n', '<C-h>', '<C-w>h', opts)
set('n', '<C-l>', '<C-w>l', opts)
set('n', '<C-k>', '<C-w>k', opts)
set('n', '<C-j>', '<C-w>j', opts)
set('n', '<D-j>', 'yyp', opts)

-- better scoll
set('n', '<C-u>', '<C-u>zz', opts)
set('v', '<C-u>', '<C-u>zz', opts)
set('n', '<C-d>', '<C-d>zz', opts)
set('v', '<C-d>', '<C-d>zz', opts)
set('n', 'n', 'nzz', opts)

-- use <space> as leader key
set('', '<Space>', '<Nop>', opts)
vim.g.maplocalleader = ' '
vim.g.mapleader = ' '

-- resize window
set('n', '<C-S-o>', ':vertical resize -2<CR>', opts)
set('n', '<C-S-p>', ':vertical resize +2<CR>', opts)

-- buffer line
-- set('n', '<tab>', [[:BufferLineCycleNext<cr>]], opts)
-- set('n', '<s-tab>', [[:BufferLineCyclePrev<cr>]], opts)
-- set('n', '<s-h>', [[:BufferLineCycleNext<cr>]], opts)
-- set('n', '<s-l>', [[:BufferLineCyclePrev<cr>]], opts)

set('n', '<tab>', [[:bn<cr>]], opts)
set('n', '<s-tab>', [[:bN<cr>]], opts)

-- telescope
set('n', '<c-p>', function()
    require('telescope.builtin').find_files(require('telescope.themes').get_dropdown {})
end, opts)

set('n', '<c-;>', function()
    require('telescope.builtin').commands(require('telescope.themes').get_dropdown {
        layout_config = {
            width = 0.7,
        },
    })
end, opts)

-- gitsigns
set('n', ']c', ':Gitsigns next_hunk<cr>', opts)
set('n', '[c', ':Gitsigns prev_hunk<cr>', opts)
